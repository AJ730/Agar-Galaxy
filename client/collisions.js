...
const Star = require('./classes/deathStar');
...
function checkForStarCollision(pData,pConfig, stars, settings){
    return new Promise((resolve, reject)=>{
        //ORB COLLISIONS
        stars.forEach((star,i)=>{
            // console.log("CHECK FOR COLLISIONS")
        // AABB Test(square)  - Axis-aligned bounding boxes
        if(pData.radius > star.radius){
            if(pData.locX + pData.radius + star.radius > star.locX 
                && pData.locX < star.locX + pData.radius + star.radius
                && pData.locY + pData.radius + star.radius > star.locY 
                && pData.locY < star.locY + pData.radius + star.radius){
            // Pythagoras test(circle)
                distance = Math.sqrt(
                    ((pData.locX - star.locX) * (pData.locX - star.locX)) + 
                    ((pData.locY - star.locY) * (pData.locY - star.locY))	
                    );
                if(distance < pData.radius + star.radius){
            //COLLISION!!!
                    
                    // we have to keep orbs updated for new players
                    // we just dont want to push them out more than we have to
                    stars.splice(i, 1, new Star(settings));
                    // can't hit more than one orb on a tick so return
                    let collisionData = updateStarDeath(star,pData);
...
function updateStarDeath(star, killed){
    killed.alive = false;
    return{
        died: killed,
        killedBy: {
...
module.exports = {checkForOrbCollisions, checkForPlayerCollisions, checkForStarCollision}